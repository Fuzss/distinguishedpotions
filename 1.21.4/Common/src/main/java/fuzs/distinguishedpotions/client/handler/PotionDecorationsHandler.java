...
public class PotionDecorationsHandler {
    ...
    public static boolean renderPotionDecorations(GuiGraphics guiGraphics, ItemStack itemStack, int itemPosX, int itemPosY) {
        Iterable<MobEffectInstance> mobEffects = itemStack.getOrDefault(DataComponents.POTION_CONTENTS,
                PotionContents.EMPTY).getAllEffects();
        int dotCount = StreamSupport.stream(mobEffects.spliterator(), false)
                .mapToInt(MobEffectInstance::getAmplifier)
                .map((int amplifierValue) -> amplifierValue + 1)
                .sum();
        if (dotCount != 0) {
            ...
            guiGraphics.fill(RenderType.guiOverlay(), startX, startY, startX + 11, startY + 2, ARGB.opaque(0));
            for (int i = 0; i < Math.min(4, dotCount); i++) {
                int color = POTION_AMPLIFIER_COLORS[Math.min((dotCount - i - 1) / 4,
                        POTION_AMPLIFIER_COLORS.length - 1)].getColor();
    ...
    public static boolean renderPotionStackSize(GuiGraphics guiGraphics, Font font, ItemStack stack, int itemPosX, int itemPosY) {
        ...
        if (stack.getCount() != 1) {
            ...
            guiGraphics.drawString(font,
                    string,
                    itemPosX + 19 - 2 - font.width(string),
                    itemPosY + 6 + 3,
                    16777215,
                    true);
            poseStack.popPose();
